...
export type <%= h.inflection.camelize(name) %> = (input: <%= h.inflection.camelize(name) %>Input, pathParams: <%= h.inflection.camelize(name) %>PathParams) => Promise<AxiosResponse<<%= h.inflection.camelize(name) %>Output>>
